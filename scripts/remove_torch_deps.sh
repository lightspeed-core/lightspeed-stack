...
printf "%s\n%s" "${FAISS_VERSION}" "${TIKTOKEN_VERSION}" | uv pip compile  - -o requirements.binary.txt --generate-hashes  --python-version 3.12 --no-deps --universal
...
mv "${INPUT_FILE}.tmp3" "$INPUT_FILE"
rm "${INPUT_FILE}.tmp" "${INPUT_FILE}.tmp2"
