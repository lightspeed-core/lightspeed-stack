...
BUILD_ARGS_FILE=".konflux/build-args-konflux.conf"
...
cleanup() {
    ...
    $CONTAINER_RUNTIME exec "$CONTAINER_NAME" subscription-manager unregister 2>/dev/null || true
...
if [[ -n "$REGISTRY_USERNAME" && -n "$REGISTRY_PASSWORD" ]]; then
    ...
    printf '%s\n' "$REGISTRY_PASSWORD" | $CONTAINER_RUNTIME exec -i "$CONTAINER_NAME" skopeo login "$REGISTRY_HOST" \
...
CONTAINER_INPUT="$(basename "$INPUT_FILE")"
CONTAINER_OUTPUT="$(basename "$OUTPUT_FILE")"

echo "Creating workdir and copying files..."
$CONTAINER_RUNTIME exec "$CONTAINER_NAME" mkdir -p "$WORKDIR"
$CONTAINER_RUNTIME cp "$INPUT_FILE" "$CONTAINER_NAME:$WORKDIR/$CONTAINER_INPUT"
...
$CONTAINER_RUNTIME exec -w "$WORKDIR" "$CONTAINER_NAME" bash -c '
    ...
    /root/.local/bin/rpm-lockfile-prototype \
        --image "'"$BASE_IMAGE"'" \
        --outfile "'"$CONTAINER_OUTPUT"'" \
        "'"$CONTAINER_INPUT"'"
'

echo "Copying output file from container..."
$CONTAINER_RUNTIME cp "$CONTAINER_NAME:$WORKDIR/$CONTAINER_OUTPUT" "$OUTPUT_FILE"
