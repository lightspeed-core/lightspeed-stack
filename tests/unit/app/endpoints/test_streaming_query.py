...
async def _test_streaming_query_endpoint_handler(mocker, store_transcript=False):
    ...
    mocker.patch("app.endpoints.streaming_query.get_llama_stack_client", return_value=mock_client)
    mocker.patch("app.endpoints.streaming_query.retrieve_response", return_value=mock_streaming_response)
    mocker.patch("app.endpoints.streaming_query.select_model_id", return_value="fake_model_id")
    mocker.patch(
        "app.endpoints.streaming_query.is_transcripts_enabled", return_value=store_transcript
    )
    mocker.patch("app.endpoints.streaming_query.retrieve_user_id", return_value="user_id_placeholder")
    mock_transcript = mocker.patch("app.endpoints.streaming_query.store_transcript")

    query_request = QueryRequest(query=query)

    # Await the async function
    response = await streaming_query_endpoint_handler(None, query_request, auth="mock_auth")
...
def test_no_available_models(mocker):
    """Test the select_model_id function with no available models."""
    mock_client = mocker.Mock()
...
def test_validate_attachments_metadata_invalid_content_type():
    """Test the validate_attachments_metadata function with invalid content type."""
    attachments = [
        Attachment(
            attachment_type="log",
...
def test_store_transcript(mocker):
    ...
    mocker.patch(
        "app.endpoints.streaming_query.construct_transcripts_path",
        return_value=mocker.MagicMock(),
    )

    # Mock the JSON to assert the data is stored correctly
    mock_json = mocker.patch("app.endpoints.streaming_query.json")
