...
class TestConversationsV2Otel:
    """OTEL instrumentation tests for conversations v2 endpoints."""
    ...
    async def test_list_span_on_success(
        ...
        mocker.patch("app.endpoints.conversations_v2.tracer", tracer)
        mock_authorization_resolvers(mocker)
        mocker.patch("app.endpoints.conversations_v2.configuration", mock_configuration)
        mock_configuration.conversation_cache.list.return_value = [
            ConversationData(
                conversation_id=VALID_CONVERSATION_ID,
                topic_summary="summary1",
                last_message_timestamp=1704067200.0,
            ),
            ConversationData(
                conversation_id="456e7890-e12b-34d5-a678-901234567890",
                topic_summary="summary2",
                last_message_timestamp=1704067201.0,
            ),
        ]
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "conversations_v2.list"
        assert span.attributes["conversations.count"] == 2
    ...
    async def test_list_span_records_error(
        ...
        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        assert spans[0].name == "conversations_v2.list"
        assert spans[0].status.status_code == StatusCode.ERROR

    @pytest.mark.asyncio
    async def test_get_span_on_success(
        self,
        mocker: MockerFixture,
        mock_configuration: MockType,
        otel: tuple[Any, InMemorySpanExporter],
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "conversations_v2.get"
    ...
    async def test_get_span_records_error_on_not_found(
        ...
        """Test that the get span records an error when conversation not found."""
        tracer, exporter = otel
        mocker.patch("app.endpoints.conversations_v2.tracer", tracer)
        mock_authorization_resolvers(mocker)
        mocker.patch("app.endpoints.conversations_v2.configuration", mock_configuration)
        ...
        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        assert spans[0].name == "conversations_v2.get"
        assert spans[0].status.status_code == StatusCode.ERROR
    ...
    async def test_delete_span_on_success(
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "conversations_v2.delete"
    ...
    async def test_delete_span_records_error(
        ...
        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        assert spans[0].name == "conversations_v2.delete"
        assert spans[0].status.status_code == StatusCode.ERROR
    ...
    async def test_update_span_on_success(
        ...
        update_request = ConversationUpdateRequest(topic_summary="New summary")
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "conversations_v2.update"
        assert span.attributes["conversations.updated"] is True

    @pytest.mark.asyncio
    async def test_update_span_records_error_on_not_found(
        self,
        mocker: MockerFixture,
        mock_configuration: MockType,
        otel: tuple[Any, InMemorySpanExporter],
    ) -> None:
        """Test that the update span records an error when conversation not found."""
        tracer, exporter = otel
        mocker.patch("app.endpoints.conversations_v2.tracer", tracer)
        mock_authorization_resolvers(mocker)
        mocker.patch("app.endpoints.conversations_v2.configuration", mock_configuration)
        ...
        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        assert spans[0].name == "conversations_v2.update"
        assert spans[0].status.status_code == StatusCode.ERROR
