...
def mock_custom_prompt_fixture(mocker: MockerFixture) -> Callable[[str], None]:
    ...
    def _set(prompt: str) -> None:
        ...
        mock_config.shields = []
        mocker.patch(
            "lightspeed_stack.app.endpoints.rlsapi_v1.configuration", mock_config
        )
...
def _setup_responses_mock(mocker: MockerFixture, create_behavior: Any) -> None:
    ...
    mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.AsyncOgxClientHolder",
...
def mock_configuration_fixture(
    ...
    mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.configuration", minimal_config
    )
...
def mock_shield_passed_fixture(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.run_shield_moderation_v2",
...
async def test_infer_extract_token_usage_on_failure_depends_on_verbose(
    ...
    mock_extract = mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.extract_token_usage"
    )
...
def mock_quota_config_fixture(
    ...
    def _set(quota_subject: str) -> None:
        ...
        config_mock.shields = []
        mocker.patch(
            "lightspeed_stack.app.endpoints.rlsapi_v1.configuration", config_mock
        )
...
async def test_infer_quota_check_called_when_configured(
    ...
    mock_consume = mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.consume_query_tokens"
    )
...
async def test_check_shield_redaction_modifies_input(
    ...
    mocker.patch(
        "lightspeed_stack.app.endpoints.rlsapi_v1.configuration",
