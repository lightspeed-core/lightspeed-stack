...
from models.common.tools import CatalogTool, ListedMcpTool
...
def _mock_file_search_tools(
    ...
    mocker.patch(
        ...
        return_value=(
            MOCK_FILE_SEARCH_TOOLS if file_search_tools is None else file_search_tools
        ),
    )
...
@pytest.mark.asyncio
async def test_tools_includes_agent_capability_tools(
    mocker: MockerFixture,
    mock_configuration: Configuration,
    ...
    config_with_skills = mock_configuration.model_copy(
        update={"skills": mock_skills_configuration, "mcp_servers": []}
    )
    _make_app_config(mocker, config_with_skills)
    mocker.patch(
        "app.endpoints.tools.check_configuration_loaded",
        return_value=None,
    )
