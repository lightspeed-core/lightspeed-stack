...
_ogx_disrupt_once: dict[str, bool] = {"applied": False}
...
def reset_ogx_disrupt_once_tracking() -> None:
    """Reset before each feature; see ``environment.before_feature``."""
    _ogx_disrupt_once["applied"] = False
    _ogx_was_running["value"] = False


def _force_lightspeed_restart_after_ogx_disrupt(context: Context) -> None:
...
def ogx_connection_broken(context: Context) -> None:
    """Break the OGX connection by stopping the container.
    ...
    ``reset_ogx_disrupt_once_tracking`` clears the skip flag from
    ``before_feature`` and after OGX is restored (``restart_container``,
    ``_restore_ogx_service``) so the next disrupt step stops the container again.
    Tracking uses module state (not ``context`` alone) because Behave can clear
    custom attributes on ``context`` between scenarios.

    Checks whether the Docker container named "ogx" is running; if it
    ...
    if _ogx_disrupt_once["applied"]:
        ...
        _force_lightspeed_restart_after_ogx_disrupt(context)
        return
    ...
    if is_prow_environment():
        from tests.e2e.utils.prow_utils import disrupt_ogx_pod

        was_running = disrupt_ogx_pod()
        context.ogx_was_running = was_running
        _ogx_was_running["value"] = was_running
        _ogx_disrupt_once["applied"] = True
        _force_lightspeed_restart_after_ogx_disrupt(context)
        return
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            subprocess.run(["docker", "stop", "ogx"], check=True, capture_output=True)
