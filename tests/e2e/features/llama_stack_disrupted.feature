...
Feature: Llama Stack connection disrupted
  ...
  # --- lightspeed-stack-g-default.yaml (aligned with health, info, models, …) ---
...
# --- @cfg_default (noop auth; tools list needs no bearer) ---
  ...
  # --- lightspeed-stack-g-authorized.yaml (aligned with query, responses, conversations, …) ---
  ...
  Scenario: Check if LLM responds for query request with error for inability to connect to llama-stack
    ...
    And The service uses the lightspeed-stack-g-authorized.yaml configuration
      And The service is restarted
  ...
  Scenario: Responses returns error when unable to connect to llama-stack
    Given The service uses the lightspeed-stack-g-authorized.yaml configuration
      And The service is restarted
    Given The system is in default state
    And The llama-stack connection is disrupted
    And I set the Authorization header to Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6Ikpva
...
# --- still @cfg_authorized (noop-with-token; not RBAC) ---
  ...
  # --- lightspeed-stack-g-rbac.yaml (aligned with rbac.feature / rlsapi_v1_errors.feature) ---
