...
def test_2_default_configuration() -> None:
    """Test that configuration attributes are not accessible for uninitialized app."""
    cfg = AppConfig()
    assert cfg is not None
...
def test_2_configuration_is_singleton() -> None:
    """Test that configuration is singleton."""
    cfg1 = AppConfig()
    cfg2 = AppConfig()
    assert cfg1 == cfg2
...
def test_2_init_from_dict() -> None:
    """Test the configuration initialization from dictionary with config values."""
    config_dict: dict[str, Any] = {
        "name": "foo",
        "service": {
...
def test_2_init_from_dict_with_mcp_servers() -> None:
    """Test initialization with MCP servers configuration."""
    config_dict = {
        "name": "foo",
        "service": {
...
def test_2_init_from_dict_with_authorization_configuration() -> None:
    """Test initialization with authorization configuration configuration.
...
def test_2_load_proper_configuration(tmpdir: Path) -> None:
    """Test loading proper configuration from YAML file.
...
def test_2_load_configuration_with_mcp_servers(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with MCP servers."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_mcp_servers_property_empty() -> None:
    """Test mcp_servers property returns empty list when no servers configured."""
    config_dict: dict[str, Any] = {
        "name": "test",
        "service": {
...
def test_2_mcp_servers_property_with_servers() -> None:
    """Test mcp_servers property returns correct list of ModelContextProtocolServer objects."""
    config_dict = {
        "name": "test",
        "service": {
...
def test_2_configuration_not_loaded() -> None:
    """Test that accessing configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.configuration
...
def test_2_service_configuration_not_loaded() -> None:
    """Test that accessing service_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.service_configuration
...
def test_2_user_data_collection_configuration_not_loaded() -> None:
    """Test that accessing user_data_collection_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.user_data_collection_configuration
...
def test_2_mcp_servers_not_loaded() -> None:
    """Test that accessing mcp_servers before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.mcp_servers
...
def test_2_authentication_configuration_not_loaded() -> None:
    """Test that accessing authentication_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.authentication_configuration
...
def test_2_customization_not_loaded() -> None:
    """Test that accessing customization before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.customization
...
def test_2_load_configuration_with_customization_system_prompt_path(
    tmpdir: Path,
) -> None:
    """Test loading configuration from YAML file with customization."""
    system_prompt_filename = tmpdir / "system_prompt.txt"
...
def test_2_load_configuration_with_customization_system_prompt(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with system_prompt in the customization."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_configuration_with_profile_customization(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with a custom profile."""
    expected_profile = CustomProfile(path="tests/profiles/test/profile.py")
    expected_prompts = expected_profile.get_prompts()
    cfg_filename = tmpdir / "config.yaml"
...
def test_2_configuration_with_all_customizations(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with a custom profile, prompt and prompt path."""
    expected_profile = CustomProfile(path="tests/profiles/test/profile.py")
    expected_prompts = expected_profile.get_prompts()
    system_prompt_filename = tmpdir / "system_prompt.txt"
...
def test_2_configuration_with_sqlite_conversation_cache(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with conversation cache configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_configuration_with_in_memory_conversation_cache(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with conversation cache configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_configuration_with_quota_handlers_no_storage(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_configuration_with_token_history_no_storage(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_configuration_with_quota_handlers(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_load_configuration_with_azure_entra_id(tmpdir: Path) -> None:
    """Return Azure Entra ID configuration when provided in configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_2_load_configuration_with_incomplete_azure_entra_id_raises(
    tmpdir: Path,
) -> None:
    """Raise error if Azure Entra ID block is incomplete in configuration."""
    cfg_filename = tmpdir / "config.yaml"
...
def test_2_rag_id_mapping_excludes_solr_when_okp_not_configured(
    minimal_config: AppConfig,
) -> None:
    """Test that rag_id_mapping does not include OKP/Solr when OKP is not in rag config."""
    assert minimal_config.rag_id_mapping == {}
...
def test_2_rag_id_mapping_includes_solr_when_okp_in_inline() -> None:
    """Test that rag_id_mapping includes OKP/Solr mapping when OKP is in rag.inline."""
    cfg = AppConfig()
    cfg.init_from_dict(
        {
...
def test_2_rag_id_mapping_includes_solr_when_okp_in_tool() -> None:
    """Test that rag_id_mapping includes OKP/Solr mapping when OKP is in rag.tool."""
    cfg = AppConfig()
    cfg.init_from_dict(
        {
...
def test_2_rag_id_mapping_with_byok(tmp_path: Path) -> None:
    """Test that rag_id_mapping builds correct mapping from BYOK config."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_2_rag_id_mapping_with_byok_and_okp(tmp_path: Path) -> None:
    """Test that rag_id_mapping includes both BYOK and OKP entries when OKP is configured."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_2_resolve_index_name_with_mapping(minimal_config: AppConfig) -> None:
    """Test resolve_index_name uses mapping when available."""
    mapping = {"vs-x": "user-friendly-name"}
    assert minimal_config.resolve_index_name("vs-x", mapping) == "user-friendly-name"


def test_2_resolve_index_name_passthrough(minimal_config: AppConfig) -> None:
    """Test resolve_index_name passes through unmapped IDs."""
    assert minimal_config.resolve_index_name("vs-unknown", {}) == "vs-unknown"


def test_2_rag_id_mapping_not_loaded() -> None:
    """Test that rag_id_mapping raises when config not loaded."""
    cfg = AppConfig()
    cfg._configuration = None
    with pytest.raises(LogicError):
...
def test_2_score_multiplier_mapping_empty_when_no_byok(
    minimal_config: AppConfig,
) -> None:
    """Test that score_multiplier_mapping returns empty dict when no BYOK RAG configured."""
    assert minimal_config.score_multiplier_mapping == {}
...
def test_2_score_multiplier_mapping_with_byok_defaults(tmp_path: Path) -> None:
    """Test that score_multiplier_mapping uses default multiplier when not specified."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_2_score_multiplier_mapping_with_custom_values(tmp_path: Path) -> None:
    """Test that score_multiplier_mapping builds correct mapping with custom values."""
    db_file1 = tmp_path / "test1.db"
    db_file1.touch()
    db_file2 = tmp_path / "test2.db"
...
def test_2_score_multiplier_mapping_not_loaded() -> None:
    """Test that score_multiplier_mapping raises when config not loaded."""
    cfg = AppConfig()
    cfg._configuration = None
    with pytest.raises(LogicError):
...
@pytest.mark.parametrize("config_dict", wrong_configurations)
def test_2_init_from_dict_fake_data(config_dict: dict[str, Any]) -> None:
    """Test the configuration initialization from dictionary with config values."""
    with pytest.raises((ValueError, InvalidConfigurationError)):
        # try to initialize the app config and load configuration from a Python
        # dictionary
...
def test_3_default_configuration() -> None:
    """Test that configuration attributes are not accessible for uninitialized app."""
    cfg = AppConfig()
    assert cfg is not None
...
def test_3_configuration_is_singleton() -> None:
    """Test that configuration is singleton."""
    cfg1 = AppConfig()
    cfg2 = AppConfig()
    assert cfg1 == cfg2
...
def test_3_init_from_dict() -> None:
    """Test the configuration initialization from dictionary with config values."""
    config_dict: dict[str, Any] = {
        "name": "foo",
        "service": {
...
def test_3_init_from_dict_with_mcp_servers() -> None:
    """Test initialization with MCP servers configuration."""
    config_dict = {
        "name": "foo",
        "service": {
...
def test_3_init_from_dict_with_authorization_configuration() -> None:
    """Test initialization with authorization configuration configuration.
...
def test_3_load_proper_configuration(tmpdir: Path) -> None:
    """Test loading proper configuration from YAML file.
...
def test_3_load_configuration_with_mcp_servers(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with MCP servers."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_mcp_servers_property_empty() -> None:
    """Test mcp_servers property returns empty list when no servers configured."""
    config_dict: dict[str, Any] = {
        "name": "test",
        "service": {
...
def test_3_mcp_servers_property_with_servers() -> None:
    """Test mcp_servers property returns correct list of ModelContextProtocolServer objects."""
    config_dict = {
        "name": "test",
        "service": {
...
def test_3_configuration_not_loaded() -> None:
    """Test that accessing configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.configuration
...
def test_3_service_configuration_not_loaded() -> None:
    """Test that accessing service_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.service_configuration
...
def test_3_user_data_collection_configuration_not_loaded() -> None:
    """Test that accessing user_data_collection_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.user_data_collection_configuration
...
def test_3_mcp_servers_not_loaded() -> None:
    """Test that accessing mcp_servers before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.mcp_servers
...
def test_3_authentication_configuration_not_loaded() -> None:
    """Test that accessing authentication_configuration before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.authentication_configuration
...
def test_3_customization_not_loaded() -> None:
    """Test that accessing customization before loading raises an error."""
    cfg = AppConfig()
    with pytest.raises(LogicError, match="logic error: configuration is not loaded"):
        c = cfg.customization
...
def test_3_load_configuration_with_customization_system_prompt_path(
    tmpdir: Path,
) -> None:
    """Test loading configuration from YAML file with customization."""
    system_prompt_filename = tmpdir / "system_prompt.txt"
...
def test_3_load_configuration_with_customization_system_prompt(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with system_prompt in the customization."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_configuration_with_profile_customization(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with a custom profile."""
    expected_profile = CustomProfile(path="tests/profiles/test/profile.py")
    expected_prompts = expected_profile.get_prompts()
    cfg_filename = tmpdir / "config.yaml"
...
def test_3_configuration_with_all_customizations(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with a custom profile, prompt and prompt path."""
    expected_profile = CustomProfile(path="tests/profiles/test/profile.py")
    expected_prompts = expected_profile.get_prompts()
    system_prompt_filename = tmpdir / "system_prompt.txt"
...
def test_3_configuration_with_sqlite_conversation_cache(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with conversation cache configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_configuration_with_in_memory_conversation_cache(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with conversation cache configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_configuration_with_quota_handlers_no_storage(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_configuration_with_token_history_no_storage(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_configuration_with_quota_handlers(tmpdir: Path) -> None:
    """Test loading configuration from YAML file with quota handlers configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_load_configuration_with_azure_entra_id(tmpdir: Path) -> None:
    """Return Azure Entra ID configuration when provided in configuration."""
    cfg_filename = tmpdir / "config.yaml"
    with open(cfg_filename, "w", encoding="utf-8") as fout:
        fout.write("""
...
def test_3_load_configuration_with_incomplete_azure_entra_id_raises(
    tmpdir: Path,
) -> None:
    """Raise error if Azure Entra ID block is incomplete in configuration."""
    cfg_filename = tmpdir / "config.yaml"
...
def test_3_rag_id_mapping_excludes_solr_when_okp_not_configured(
    minimal_config: AppConfig,
) -> None:
    """Test that rag_id_mapping does not include OKP/Solr when OKP is not in rag config."""
    assert minimal_config.rag_id_mapping == {}
...
def test_3_rag_id_mapping_includes_solr_when_okp_in_inline() -> None:
    """Test that rag_id_mapping includes OKP/Solr mapping when OKP is in rag.inline."""
    cfg = AppConfig()
    cfg.init_from_dict(
        {
...
def test_3_rag_id_mapping_includes_solr_when_okp_in_tool() -> None:
    """Test that rag_id_mapping includes OKP/Solr mapping when OKP is in rag.tool."""
    cfg = AppConfig()
    cfg.init_from_dict(
        {
...
def test_3_rag_id_mapping_with_byok(tmp_path: Path) -> None:
    """Test that rag_id_mapping builds correct mapping from BYOK config."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_3_rag_id_mapping_with_byok_and_okp(tmp_path: Path) -> None:
    """Test that rag_id_mapping includes both BYOK and OKP entries when OKP is configured."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_3_resolve_index_name_with_mapping(minimal_config: AppConfig) -> None:
    """Test resolve_index_name uses mapping when available."""
    mapping = {"vs-x": "user-friendly-name"}
    assert minimal_config.resolve_index_name("vs-x", mapping) == "user-friendly-name"


def test_3_resolve_index_name_passthrough(minimal_config: AppConfig) -> None:
    """Test resolve_index_name passes through unmapped IDs."""
    assert minimal_config.resolve_index_name("vs-unknown", {}) == "vs-unknown"


def test_3_rag_id_mapping_not_loaded() -> None:
    """Test that rag_id_mapping raises when config not loaded."""
    cfg = AppConfig()
    cfg._configuration = None
    with pytest.raises(LogicError):
...
def test_3_score_multiplier_mapping_empty_when_no_byok(
    minimal_config: AppConfig,
) -> None:
    """Test that score_multiplier_mapping returns empty dict when no BYOK RAG configured."""
    assert minimal_config.score_multiplier_mapping == {}
...
def test_3_score_multiplier_mapping_with_byok_defaults(tmp_path: Path) -> None:
    """Test that score_multiplier_mapping uses default multiplier when not specified."""
    db_file = tmp_path / "test.db"
    db_file.touch()
    cfg = AppConfig()
...
def test_3_score_multiplier_mapping_with_custom_values(tmp_path: Path) -> None:
    """Test that score_multiplier_mapping builds correct mapping with custom values."""
    db_file1 = tmp_path / "test1.db"
    db_file1.touch()
    db_file2 = tmp_path / "test2.db"
...
def test_3_score_multiplier_mapping_not_loaded() -> None:
    """Test that score_multiplier_mapping raises when config not loaded."""
    cfg = AppConfig()
    cfg._configuration = None
    with pytest.raises(LogicError):
