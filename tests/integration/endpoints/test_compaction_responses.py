...
class TestResponsesConversationCompaction:
    ...
    @pytest.mark.asyncio
    async def test_responses_compaction_no_conversation_id_no_compaction(
        self,
        test_config: AppConfig,
        test_auth: AuthTuple,
        mock_ogx_client: AsyncMockType,
        ...
        """Request without conversation ID skips compaction entirely.

        Verifies:
        - No summarization or marker write when no conversation ID is provided
        """
        enable_compaction(test_config)
        user_id, _, _, _ = test_auth
        create_existing_conversation(patch_db_session, user_id)
        ...
        mock_summarize = _setup_responses_compaction_mocks(mocker, [])

        # Same setup as test_responses_compaction_triggers_summarization to make sure that the
        # compaction is not triggered because the request does not contain conversation ID.
