...
async def _await_lock_contention(conv_id: str, expected_waiters: int = 2) -> None:
    """Wait until the per-conversation lock has the expected number of waiters."""
    from utils.conversation_compaction import _conversation_locks
...
class TestQueryConversationCompation:
    ...
    async def test_query_compaction_triggers_summarization(
        ...
        _assert_marker_count(mock_conversation_store, CONV_ID_LLAMA, 1)
        ...
        # Check the status of the conversation we provide to the model is what we expect
        ...
        # Check the status of the conversation store is what we expect
        items_from_store = await _collect_items(mock_conversation_store, CONV_ID_LLAMA)
        assert len(items_from_store) == 7
        expected = items + [
            _marker(DEFAULT_SUMMARY_TEXT),
            _msg("user", "What else can you help with?"),
            _msg("assistant", DEFAULT_MODEL_RESPONSE),
        ]
        assert _verify_store_content(items_from_store, expected)
    ...
    async def test_query_compaction_small_conversation_no_compaction(
        ...
        mock_summarize.assert_not_called()
        _assert_marker_count(mock_conversation_store, CONV_ID_LLAMA, 0)

        agent_params = mock_query_agent.build_agent_mock.call_args[0][1]
        assert agent_params.omit_conversation is False
        assert isinstance(agent_params.input, str)
        # No need to check input because without triggering compaction,
        # it's just the simple user input.

        # No need to check the conversation store because when we're not in
        # the compaction mode, the conversation store in organized by OGX.
    ...
    async def test_query_conversation_compaction_additive_summarization(
        ...
        mock_summarize.assert_awaited_once()
        _assert_marker_count(mock_conversation_store, CONV_ID_LLAMA, 2)
    ...
    async def test_query_conversation_compaction_blocking_concurrent_request_with_same_id(
        ...
        try:
            await asyncio.wait_for(_await_lock_contention(CONV_ID_LLAMA), 10)
        except TimeoutError:
    ...
    async def test_query_compaction_recursive_fold(
        ...
        items = [
            _marker("summary of turns 1-2"),
            _marker("summary of turns 3-4"),
            _msg("user", "question five " * 20),
            _msg("assistant", "answer five " * 20),
        ]
        await mock_conversation_store.create(conversation_id=CONV_ID_LLAMA, items=items)

        mock_cache, mock_summarize, mock_resummarize = _setup_fold_mocks(
            mocker,
