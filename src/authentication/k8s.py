...
def _resolve_kube_admin_uid(
    user: kubernetes.client.V1UserInfo, start_time: float
) -> str:
    """Resolve the effective UID for a Kubernetes user.

    For ``kube:admin`` users, the UID is replaced with the cluster ID
    to provide a stable identifier. For all other users, the original
    UID is returned unchanged.

    Args:
        user: Kubernetes user info from token review.
        start_time: Monotonic timestamp for auth duration metrics.

    Returns:
        The resolved user UID string.

    Raises:
        HTTPException: If cluster ID resolution fails for kube:admin users.
    """
    if user.username != "kube:admin":
        return cast(str, user.uid)
    try:
        return K8sClientSingleton.get_cluster_id()
        ...
        raise HTTPException(**resp.model_dump()) from e
...
class K8SAuthDependency(AuthInterface):  # pylint: disable=too-few-public-methods
    ...
    async def __call__(self, request: Request) -> AuthTuple:
        ...
        user_uid = _resolve_kube_admin_uid(user, start_time)
