...
def extract_token_usage(response_usage: Any) -> tuple[int, int]:
    ...
    Returns:
        Tuple of (input_tokens, output_tokens), defaulting to 0 for missing/None values
    """
    if not response_usage:
        return 0, 0

    if isinstance(response_usage, dict):
        input_tokens = response_usage.get("input_tokens") or 0
        output_tokens = response_usage.get("output_tokens") or 0
    else:
        input_tokens = getattr(response_usage, "input_tokens", None) or 0
        output_tokens = getattr(response_usage, "output_tokens", None) or 0
