...
TLS_CIPHERS: dict[TLSProfiles, list[str]] = {
...
def ssl_tls_version(
    ...
    """Convert TLS protocol version string into its ssl.TLSVersion equivalent."""
    if tls_protocol_version is None:
        return None
    tls_versions = {
...
) -> Optional[TLSProtocolVersion]:
    """Retrieve minimal TLS version for the profile or from specified configuration."""
    if specified_tls_version is not None:
        try:
            return TLSProtocolVersion(specified_tls_version)
        except ValueError:
            logger.warning(
                "Invalid TLS version '%s', using profile default", specified_tls_version
            )
    return MIN_TLS_VERSIONS.get(tls_profile)
...
def ciphers_as_string(
    ...
    """Retrieve ciphers as one string for custom list or TLS profile-based list."""
