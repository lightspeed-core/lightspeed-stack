# Lightspeed Core Stack

## Lunch & Learn

---

![LCORE](images/lcore.jpg)

---

## 1. What Problem Are We Solving?

- Why teams can't just "call an LLM" in production

- Added value of LCS:

    LCS comes with everything a team needs out of the box: authentication, authorization, quotas, caching, OKP integration, RAG, observability, safety shields, multi-provider support. So teams don't have to build or integrate those pieces themselves

---

## 2. Success Stories: 4 Teams in Production

- Ansible Lightspeed
- DevHub Lightspeed
- AI-Assisted Installer
- RHEL Lightspeed

---

## 3. Architecture at a Glance

- Library mode vs. Server mode
- Query, Streaming Query, Responses API

---

![Architecture](images/ogx_arch.png)

---

### OGX as a library

![LS1](images/ogx_as_library.svg)

---

### OGX as a service

![LS2](images/ogx_as_service.svg)

---

### Running inside container

![LS3](images/ogx_in_container.svg)

---

## 4. Core Capabilities — AI

- **Multi-provider LLM access** — OpenAI, Azure, VertexAI, WatsonX, Bedrock, vLLM (RHOAI/RHAIIS) 
- **Responses API compatible**
- **Streaming**
- **RAG** — Inline/tool-based search , OKP and BYOK
- **Tool Calling** — MCP server integration
- **A2A Protocol** 
- **Guardrails**

---

## 4. Core Capabilities — Platform & Operations

- **Authentication & Authorization** — 6 pluggable auth modules (K8s, JWT, API key, RH Identity), RBAC with 30+ actions
- **Quotas & Observability** — Per-user and cluster-wide token limits, scheduled resets, Prometheus metrics for API, quota
- **Conversation Management** — Full conversation history storage and retrieval, multiple cache backends (PostgreSQL, SQLite, in-memory)

---

## 5. Getting Started / How to Onboard

- Configuration walkthrough: `lightspeed-stack.yaml` + `run.yaml`
- Deployment options: CLI, Docker Compose (dev), Konflux builds
- Documentation: README and repo docs
- Community: Slack #lightspeed-forum

---

## 6. Live Demo


---

## 7. What's Next / Roadmap

- Upcoming features or integrations
- How to contribute or request features

---

## 8. Q&A

---

<?xml version="1.0"?>
<svg xmlns="http://www.w3.org/2000/svg" width="340" height="320" viewBox="0 0 320 320" fill="none">
    <rect x="320" y="300" width="20" height="20" fill="black" stroke="white">
        <animate
            attributeType="XML"
            attributeName="fill"
            values="black;black;white;white;black"
            dur="0.5s"
            repeatCount="indefinite"/>
    </rect>
</svg>

