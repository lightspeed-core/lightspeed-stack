...
- The tracing SDK is registered when the primary configuration toggle and mandatory export settings are satisfied.  
- When tracing is enabled, mandatory export fields in YAML—**endpoint**, **protocol**, and **service name**—must be provided to allow startup and proper OTLP export.  
- Standard **`OTEL_*`** environment variables can be used to supply sampling, OTLP headers, TLS paths, and other non-mandatory OpenTelemetry options while tracing is active.
...
- **Startup:** When `enabled: true`, fail fast or refuse tracing if mandatory **`export`** fields are missing or invalid.
...
- Default `enabled: false` preserves behavior for existing deployments.
...
| `src/app/main.py` | Lifespan: initialize / flush tracing (create separate module for the logic). |
| `src/models/config.py`, `src/configuration.py` | `OpenTelemetryConfiguration` with **`export`** validation when `enabled`. |
