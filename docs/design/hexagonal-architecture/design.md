...
2. **LCORE is now a multi-actor, multi-protocol service.** We already expose, or are actively building:
   - **Driving (primary) actors:** the REST API (`/v1/query`, `/v1/streaming_query`, the OpenAI-compatible Responses API), the A2A JSON-RPC protocol, and increasingly **agents as first-class callers**. Streaming responses makes the "request in, response out" assumption of a plain three-layer app inadequate.
...
The domain also owns **its own models** — `ConversationData`, `ConversationDetails`, `ConversationTurn`, `Message`, `ToolCallSummary`, `ToolResultSummary` etc — rather than OGX SDK objects or Pydantic request/response bodies. (These are Pydantic models today, living in `models/common/`; whether the domain keeps Pydantic or maps to plain types is Open Question #2.)
